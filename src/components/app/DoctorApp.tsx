...
// ===== MAS 卒中运动功能评估量表（在线填写 / OCR 上传） =====
type MASItem = {
  key: string;
  name: string;
  scoreRef?: boolean; // 不计入总分
  levels: string[]; // 0-6 描述
};

const MAS_ITEMS: MASItem[] = [
  {
    key: "supineToSide",
    name: "仰卧至侧卧",
    levels: [
      "完全依赖",
      "经健侧上下肢帮助翻至侧卧，无屈膝",
      "主动将腿跨向对侧带动下半身侧卧，上肢滞后",
      "两上肢抬高转向一侧，一侧下肢主动跨向对侧",
      "主动将一侧上肢转向对侧，身体随之侧卧",
      "移动上下肢翻至侧卧，但失去平衡",
      "3 秒内翻至侧卧，无需手帮助",
    ],
...
// 模拟患者已有评分（来自上次沟通 / 默认建议）
const MAS_DEFAULT_SCORES: Record<string, number> = {
  supineToSide: 3,
  supineToSit: 2,
  sitBalance: 2,
  sitToStand: 1,
...
const MASForm = ({ onSummaryChange }: { onSummaryChange?: (s: { total: number; level: string }) => void }) => {
  const [scores, setScores] = useState<Record<string, number>>(MAS_DEFAULT_SCORES);
  ...
  const total = MAS_ITEMS.filter((i) => !i.scoreRef).reduce((s, i) => s + (scores[i.key] ?? 0), 0);
  const level =
    total > 33 ? "轻度运动障碍" : total >= 17 ? "中度运动障碍" : "重度运动障碍";
  const levelColor =
    total > 33 ? "text-success" : total >= 17 ? "text-warning" : "text-destructive";
  ...
  const handleOCR = () => {
    ...
    setTimeout(() => {
      // 模拟 OCR 识别后的填充
      setScores({
        supineToSide: 4, supineToSit: 3, sitBalance: 3, sitToStand: 2,
        walking: 1, upperLimb: 2, hand: 2, fineHand: 1, tone: 4,
      });
      setNotes({ walking: "纸质表显示需 1 人监护尝试迈步" });
      setOcrLoading(false);
      setOcrDone(true);
      toast.success("OCR 识别完成 · 已自动填充评分，可逐项修改");
    }, 1200);
  };
    ...
    <div className="bg-card rounded-2xl shadow-card overflow-hidden">
      <div className="px-4 pt-4 pb-3 border-b border-border/60">
        <div className="flex items-center justify-between gap-2">
          <div>
            <div className="text-[13px] font-semibold">MAS 卒中运动功能评估</div>
            <div className="text-[10px] text-muted-foreground mt-0.5">8 项计分（0–6 分） · 全身肌张力仅参考</div>
          </div>
          <button
            onClick={handleOCR}
            disabled={ocrLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl gradient-ai text-white text-[11px] font-semibold disabled:opacity-60"
          >
            <Camera className="w-3.5 h-3.5" />
            {ocrLoading ? "识别中…" : ocrDone ? "重新拍照" : "拍照上传纸质表"}
          </button>
        </div>
        <div className="mt-3 flex items-center gap-3">
          <div className="flex-1 bg-muted rounded-xl py-2 px-3">
            <div className="text-[10px] text-muted-foreground">总分（8 项）</div>
            <div className="text-2xl font-bold text-primary leading-tight">
              {total}<span className="text-xs text-muted-foreground ml-1">/ 48</span>
            </div>
          </div>
          <div className="flex-1 bg-muted rounded-xl py-2 px-3">
            <div className="text-[10px] text-muted-foreground">分级</div>
            <div className={`text-sm font-bold mt-1 ${levelColor}`}>{level}</div>
          </div>
        </div>
      </div>
      ...
      <div className="divide-y divide-border/60">
        {MAS_ITEMS.map((item) => {
          ...
          const isEditNote = editingNote === item.key;
          return (
              ...
              >
                <div className="text-left flex-1 min-w-0">
                  <div className="text-[12px] font-semibold flex items-center gap-2">
                    {item.name}
                    {item.scoreRef && <span className="text-[9px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">不计分</span>}
                  </div>
                  <div className="text-[10px] text-muted-foreground mt-0.5 truncate">
                    {score} 分 · {item.levels[score]}
                  </div>
                </div>
                <span className={`text-[11px] font-bold ${item.scoreRef ? "text-muted-foreground" : "text-primary"}`}>{score}</span>
                ...
                <div className="mt-3 space-y-2">
                  <div className="grid grid-cols-7 gap-1">
                    {item.levels.map((_, idx) => (
                      <button
                        key={idx}
                        onClick={() => setScores({ ...scores, [item.key]: idx })}
                        className={`py-1.5 rounded-lg text-[11px] font-semibold border transition-colors ${
                          score === idx
                            ? "gradient-doctor text-white border-transparent"
                  ...
                  <div className="text-[11px] leading-relaxed bg-muted/60 rounded-lg p-2">
                    <span className="text-muted-foreground">当前选项：</span>
                    {item.levels[score]}
                  </div>
                  ...
                  {isEditNote ? (
                    <textarea
                      ...
                      placeholder="补充该项备注，例如观察到的细节、患者反馈等"
                      autoFocus
                      className="w-full text-[11px] bg-background border border-border rounded-lg p-2 min-h-[60px] focus:outline-none focus:ring-1 focus:ring-primary/30"
                    />
                  ) : notes[item.key] ? (
                    <button
                      onClick={() => setEditingNote(item.key)}
                      className="w-full text-left text-[11px] bg-warning-soft border border-warning/30 rounded-lg p-2 flex items-start gap-2"
                    >
                      <Edit2 className="w-3 h-3 text-warning mt-0.5 shrink-0" />
                      <span className="flex-1">{notes[item.key]}</span>
                    </button>
                  ) : (
                    <button
                      onClick={() => setEditingNote(item.key)}
      ...
      <div className="px-4 py-3 border-t border-border/60 bg-muted/40 text-[10px] text-muted-foreground leading-relaxed">
        评定标准：&gt; 33 分轻度运动障碍 · 17–32 分中度 · 0–16 分重度。每项可点击展开修改评分并添加备注，亦可拍照上传纸质量表自动识别。
      </div>
    </div>
  );
};
